...
func (s *MemorySubSystem) Apply(cgroupPath string, pid int) error {
	if subsystemPath, err := GetCgroupPath(s.Name(), cgroupPath, false); err == nil {
		if err := ioutil.WriteFile(path.Join(subsystemPath, "tasks"), []byte(strconv.Itoa(pid)), 0644); err != nil {
